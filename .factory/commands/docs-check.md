---
description: Run public docs validation guidance and summarize failures
...
3. For `.factory/**`, workflow, prompt, or repo config changes, inspect the diff for public-safety leaks.
4. If a check fails, inspect the failure, fix only the necessary files, and rerun the failed check.
