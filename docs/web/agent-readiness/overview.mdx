---
title: Overview
description: Understand the Agent Readiness Model across Droid CLI, dashboard, and API surfaces.
keywords:
  [
    "agent readiness",
    "readiness model",
...
1. **Droid CLI:** Run the `/readiness-report` [slash command](/cli/features/readiness-report) to evaluate a repository's readiness level
...
4. **Remediation (Coming Soon):** Automatically fix failing criteria directly from the Droid CLI or dashboard
