...
### Does Alloy dynamically change the model per request?

Alloy uses a mix of session and per-request routing, strongly considering prompt cache maintenance and savings in its optimization.
...
Alloy maintains frontier-level performance on challenging benchmarks while reducing costs roughly 20–25% compared with always using top-tier models.
