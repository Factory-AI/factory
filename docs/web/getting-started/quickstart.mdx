...
keywords: ['quickstart', 'getting started', 'factory app', 'droid cli', 'factory bridge', 'bridge', 'cloud templates', 'setup', 'install', 'download', 'local machine', 'working directory']
---
...
<Steps>
  <Step title="Download the Factory App">
    <CardGroup cols={3}>
      <Card title="Mac (Apple Silicon)" icon="apple" href={`${vars.urls.app}/api/desktop?platform=darwin&architecture=arm64`}>
        M1/M2/M3
      </Card>
      <Card title="Mac (Intel)" icon="apple" href={`${vars.urls.app}/api/desktop?platform=darwin&architecture=x64`}>
        Intel
      </Card>
      <Card title="Windows" icon="windows" href={`${vars.urls.app}/api/desktop?platform=win32`}>
        Windows x64
      </Card>
    </CardGroup>
  </Step>
  ...
  <Step title="Sign In">
    Launch the <ProductName variant="factoryApp" /> and sign in with your account.
  </Step>
</Steps>
...
<Steps>
  <Step title="Set Your Working Directory">
    Set your working directory to your project folder. The <ProductName variant="factoryApp" /> connects directly to your local machine.
  </Step>
...
<CardGroup cols={2}>
  ...
  <Card title="Droid CLI" icon="terminal" href="/cli/getting-started/overview">
