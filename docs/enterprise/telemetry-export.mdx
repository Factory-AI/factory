...
| `OTEL_LOG_MESSAGE_CONTENT` | No      | Set to `true` or `1` to export message content (user/assistant messages, tool input/results) as trace spans to **your endpoint only** -- never to Factory. Requires `OTEL_TELEMETRY_ENDPOINT`; content is dropped if no endpoint is set. |
...
By default, telemetry export includes only metrics -- the text of prompts, model responses, and tool I/O is **not** exported. Metrics fan out to both Factory and your collector (see above); message content does not.

To capture message content (for example, for session auditing), set:
...
<AccordionGroup>
  ...
  <Accordion title="Oodle">
    ```bash
    export OTEL_TELEMETRY_ENDPOINT="https://<INSTANCE_ID>-otlp.collector.oodle.ai"
    export OTEL_TELEMETRY_HEADERS="X-API-KEY=<API_KEY>,X-OODLE-INSTANCE=<INSTANCE_ID>"
    ```

    Get your `<INSTANCE_ID>` and `<API_KEY>` from the Factory Droid tile under **Settings → Integrations → AI Observability** in the Oodle UI. Droid appends `/v1/metrics` to the endpoint. Once telemetry is flowing, view it under **AI Cost Management → Factory Droid** in the Oodle sidebar. See the [Oodle Factory Droid guide](https://docs.oodle.ai/ai-cost-management/factory) for details.
  </Accordion>
</AccordionGroup>
