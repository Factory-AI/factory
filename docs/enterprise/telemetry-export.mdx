...
<Note>
The `otel_telemetry_collection` feature flag must be enabled for your organization. Contact your Factory account team to enable it.
</Note>
...
- Factory already collects product-analytics metrics and ships them to its own OTEL collector.
- When `OTEL_TELEMETRY_ENDPOINT` is set, the same metrics batch is also sent to your endpoint in the same export cycle via a fan-out exporter -- no extra timers, no duplication of metric readers.
...
- The feature requires the `otel_telemetry_collection` feature flag to be enabled for your org.
