...
<Info>
  **Enterprise Feature** — The audit log is available to **Enterprise organization owners only**. Owners can view the audit log in the Factory web app under Organization Settings, or via the public API. [Contact us](mailto:sales@factory.ai) to learn more.
</Info>
...
<Note>
  For `workos_event` source events, the payload contains only a filtered projection of the original event (`workosEventType`, `role`, `status`). Raw identity provider data is never exposed in the audit log.
</Note>
