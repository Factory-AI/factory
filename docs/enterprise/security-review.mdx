...
`/missions` builds a plan that partitions the codebase by module, surface area, and trust boundary, pulls in the `security-review` and `deep-security-review` skills, and runs the audit through Mission Control with milestone-level validation. The output is a consolidated report of validated findings, grouped by severity.

The planning phase is where missions earn their depth. A well-scoped audit plan -- with milestones around authentication, data handling, dependencies, and LLM surfaces -- produces materially better results than a one-shot scan.
...
If you do not need the full mission workflow, you can also run an audit directly inside a Droid session:
...
To run a full repository scan from a PR comment with [Droid Action](https://github.com/Factory-AI/droid-action):
