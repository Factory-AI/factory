...
For the most thorough security results, run the audit inside a [Mission](/features/missions/overview). Missions plan the audit upfront, fan out work across orchestrated agents, and validate findings at each milestone, which produces dramatically deeper coverage than a single-session run.
...
PR security reviews use the same [`review_depth`](/guides/droid-exec/code-review#review-depth) preset as code review, so you don't need to choose a model.
...
- [Missions](/features/missions/overview): Plan and orchestrate large multi-step work, including thorough audits.
