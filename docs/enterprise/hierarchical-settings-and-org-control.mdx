...
- **AutoWiki Cloud Sync** (`wikiCloudSync`) – Controls whether AutoWiki content can be generated and stored in Factory's cloud. When disabled, all AutoWiki API endpoints are blocked and the Droid CLI skips cloud upload and GitHub wiki sync. Enabled by default. See [AutoWiki](/cli/features/wiki/overview#autowiki-cloud-sync).
