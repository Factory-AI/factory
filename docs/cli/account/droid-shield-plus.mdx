...
description: AI-powered security scanning for prompt injection, secrets detection, and sensitive data protection for enterprise organizations.
keywords: ['droid shield plus', 'prompt injection', 'secrets detection', 'dlp', 'enterprise security', 'ai security']
---
...
| **Delivery** | Built in | Enterprise add-on |
...
<Steps>
  ...
  <Step title="Verify Activation">
    You'll see a subtitle confirming that AI-powered security scanning is enabled for prompt injection, sensitive data, and toxic content.
  </Step>
</Steps>
...
Droid Shield Plus is an enterprise add-on administered by your organization:

- **Real-time analysis** — Prompts and code are scanned in real time with minimal latency
- **Enterprise controls** — Availability and policy enforcement are managed for enabled organizations
