...
* **hooks**: Array of commands to execute when the pattern matches
  ...
  * `command`: The bash command to execute (can use `$DROID_PROJECT_DIR`
...
<Warning>
  **Always use absolute paths** for hook commands and scripts, not relative paths. 
  ...
  Use `"$DROID_PROJECT_DIR"/path/to/script.sh` for project-relative scripts or full paths like 
...
You can use the environment variable `DROID_PROJECT_DIR` (only available when
    ...
    "PostToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "\"$DROID_PROJECT_DIR\"/.factory/hooks/check-style.sh"
          }
        ]
      }
    ]
...
[Plugins](/cli/configuration/plugins) can provide hooks that integrate seamlessly with your user and project hooks. Plugin hooks are automatically merged with your configuration when plugins are enabled.
...
* `${DROID_PROJECT_DIR}`: Project root directory (same as for project hooks)
...
* `Task` - Subagent tasks (see [subagents documentation](/cli/user-guides/specification-mode))
* `Bash` - Shell commands
...
Runs when the main Droid agent has finished responding. Does not run if
the stoppage occurred due to a user interrupt.

### SubagentStop

Runs when a Droid subagent (Task tool call) has finished responding.
...
SessionStart hooks have access to the `DROID_ENV_FILE` environment variable, which provides a file path where you can persist environment variables for subsequent bash commands.
...
if [ -n "$DROID_ENV_FILE" ]; then
  echo 'export NODE_ENV=production' >> "$DROID_ENV_FILE"
  echo 'export API_KEY=your-api-key' >> "$DROID_ENV_FILE"
  echo 'export PATH="$PATH:./node_modules/.bin"' >> "$DROID_ENV_FILE"
fi
...
Any variables written to this file will be available in all subsequent bash commands that Droid executes during the session.

<Note>
  `DROID_ENV_FILE` is only available for SessionStart hooks. Other hook types do not have access to this variable.
</Note>
...
| `SubagentStop`     | Blocks stoppage, shows stderr to Droid subagent                   |
...
4. **Use absolute paths** - Always specify full paths for scripts. Never use relative paths like `./script.sh` or `hooks/script.sh` as they may resolve incorrectly depending on Droid's current directory. Use `"$DROID_PROJECT_DIR"/path/to/script.sh` for project scripts or full paths like `/usr/local/bin/script.sh` or `~/.factory/hooks/script.sh` for global scripts.
...
* **Environment**: Runs in current directory with Droid's environment
  * The `DROID_PROJECT_DIR` environment variable is available and contains the
    absolute path to the project root directory (where Droid was started)
  * The `DROID_REMOTE` environment variable indicates whether the hook is running in a remote (web) environment (`"true"`) or local CLI environment (not set or empty). Use this to run different logic based on execution context.
