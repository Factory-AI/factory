...
For scripting and automation, use `droid mcp add`. Droid supports three transports: **http** (streamable HTTP remote endpoints), **sse** (Server-Sent Events remote endpoints), and **stdio** (local processes).
...
SSE servers are remote MCP endpoints that stream responses over Server-Sent Events — use this transport when a server exposes an SSE endpoint instead of streamable HTTP. Arguments mirror HTTP servers; only `--type` changes.
...
- **oauth**: Optional OAuth overrides for the remote server (object). Supported fields:
  - **scopes**: OAuth scopes to request (array)
  - **clientId**: OAuth client ID
  - **clientSecret**: OAuth client secret
  - **callbackPort**: Local port used for the OAuth callback
