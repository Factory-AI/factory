...
- **Safety floor.** `high` always requires confirmation, and any tool that isn't read-only (destructive, or missing safety metadata) is forced to `high` regardless of the rule — you can relax confirmation for read-only tools but never auto-approve destructive ones.
