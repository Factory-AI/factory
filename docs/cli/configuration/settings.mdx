...
Higher effort increases latency and cost. Which levels are available — and which is the default — depend on the model, so check the **Reasoning** column in [Available Models](/models). (`dynamic` is an internal adaptive setting and is not selected manually.)
