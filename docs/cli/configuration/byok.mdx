...
### Connect to Deep Infra, Fireworks, Hugging Face, or Other Providers

Access specialized or optimized model deployments:
...
Check your provider's documentation for available model IDs and base URLs.
...
### Run Local Models (Ollama, vLLM, etc.)
...
```json
{
  "custom_models": [
    {
      "model_display_name": "Qwen:4b [Local]",
      "model": "qwen3:4b",
...
<Tip>
  Local servers often don't require API keys, but the `api_key` field is still required. Use any placeholder value.
</Tip>
