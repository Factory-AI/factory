...
**Background context (Droid-only):**

```yaml
---
name: legacy-api-context
description: Context about our legacy REST API patterns
user-invocable: false
---

When working with the legacy API in `src/api/v1/`:
- All endpoints use XML responses (not JSON)
- Authentication uses custom headers, not OAuth
- Rate limiting is handled at the nginx layer
```
