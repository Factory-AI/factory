...
Agents load **all** `AGENTS.md` files along the path from your repo root down to your current working directory, plus a personal override. Every file is concatenated into the agent's context — discovery is **additive, not first-match**.

At session start, agents load (in this order):

1. Personal override: `~/.factory/AGENTS.md`
2. Each `AGENTS.md` from the **repo root** down to the **current working directory**

In addition, when the agent reads a file deeper in the tree via the `Read` tool, any previously unseen `AGENTS.md` files along the path to that file are loaded on demand.

<Note>
  Multiple files coexist. If their instructions conflict, the file **closest to
  the working directory (or to the file being edited) takes precedence** over
  files higher up the hierarchy. Non-conflicting instructions from parent files
  still apply.
</Note>
...
<Steps>
  <Step title="Ingestion">
    On task start, agents load every AGENTS.md from the repo root down to the
    current working directory (plus the personal `~/.factory/AGENTS.md`
    override) into their context window.
  </Step>
  <Step title="Planning">
