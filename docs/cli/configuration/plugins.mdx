...
`ref` and `sha` are supported on the `github`, `url`, and `git-subdir` source types. They are mutually informative — set `sha` for hard pins, `ref` to follow a branch or tag.
