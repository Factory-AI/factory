...
- **MCP tools** -- filesystem and network requests are checked; locally launched servers receive a minimal environment so host secrets are not exposed
...
- **Your settings files are protected.** A sandboxed process can never write to Droid's own configuration (e.g. `settings.json`), so it cannot modify its own sandbox policy. 
