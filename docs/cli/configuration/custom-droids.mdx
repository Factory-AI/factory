...
| `mcpServers`      | Optional. Array of [MCP server](/cli/configuration/mcp) names whose tools should be available to this droid. When set, only tools from the listed servers are exposed to the subagent; omit to fall back to your global MCP tool availability. |
...
### Scoping MCP servers to a droid
...
---
name: mcp-reviewer
description: Reviews diffs with Chrome DevTools context
model: inherit
tools:
  - Read
mcpServers:
...
Use the configured MCP server when needed.
```

Notes:

- Server names must match entries already configured in `~/.factory/mcp.json` or `.factory/mcp.json`.
