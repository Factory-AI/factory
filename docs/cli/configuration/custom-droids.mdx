...
| `edit`      | `Create`, `Edit`, `MultiEdit`            | Code generation and modifications  |
...
You can import agents created in Claude Code (from `~/.claude/agents/`) as custom droids. This lets you reuse your existing Claude Code agents with the Factory Droid system.

### How to import

1. Run `/droids` to open the Droids menu
2. Press **I** to start the import flow
3. The CLI scans both project and personal Claude Code locations:
   - **Project scope**: `~/.claude/agents/` (workspace-specific agents)
   - **Personal scope**: `~/.claude/agents/` (personal agents)
4. Review the list of available agents:
   - Agents marked `(already exists)` are skipped
   - Pre-select agents you want to import
...
> code-reviewer (gpt-5-codex)
  ...
  Location: Personal  •  Tools: 1 selected  Invalid tools: Write, NotebookEdit
...
- These should be replaced with equivalent Factory tools (e.g., `Edit`, `Create`, `WebSearch`)
