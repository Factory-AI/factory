...
- The resolved level is always **clamped to the organization's Maximum Autonomy Level** — an explicit setting can never exceed the enterprise cap.
- When the parent session is in **Spec Mode**, subagents are restricted to read-only operations and low-risk shell commands; file edits, file creation, and spawning further subagents are disabled.
...
## 6 · Creating your own custom droid
...
## 8.5 · Importing Claude Code subagents
...
- **Leverage Claude Code agents** – import your existing Claude Code agents (see the "Importing Claude Code subagents" section below) to reuse them as custom droids in Factory.
