...
| `mcpServers`      | Optional. Array of configured MCP server names (e.g., `["linear", "github"]`) whose tools this droid may use. Servers must already be configured in your [MCP settings](/cli/configuration/mcp). See [Selecting MCP servers](#selecting-mcp-servers). |
...
Use the `mcpServers` field to grant a droid access to specific [MCP servers](/cli/configuration/mcp) you've already configured. Listing a server enables that server's MCP tools for the droid, without exposing every other server connected to your session.
...
- `mcpServers` references servers by the names they're registered under in `mcp.json` (user or project level), and grants the droid every MCP tool those servers expose.
