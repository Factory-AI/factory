...
- Rename a computer
- Delete a computer
- Open the detail page for status and connection information

Platform-managed computers also expose provisioning details, resource metrics, and remote update flows. BYOM machines surface the controls relevant to remote access and local machine registration.
...
- Setup uses `droid computer register [name]` and `droid daemon --remote-access`
- Traffic is routed through Factory's relay service, so no inbound ports need to be opened

For the full BYOM setup flow, Remote Access guidance, and management notes, see [Bring Your Own Machine (BYOM)](/cli/features/droid-computers-byom).
...
- **Managed Computers** — Git credentials are configured automatically for authenticated repository access. When you add new GitHub App installations or change repository access, credentials are refreshed automatically on the next session connection.
