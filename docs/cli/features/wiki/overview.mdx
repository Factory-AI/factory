...
description: AutoWiki keeps repository Wikis up to date in the Factory App and GitHub Wiki.
...
<Steps>
  ...
  <Step title="Store in the Factory App">
    ...
    browsable at <a href={`${vars.urls.app}/wiki`}>app.factory.ai/wiki</a>. Each
...
Organizations that need to prevent AutoWiki content from being stored in the <ProductName variant="factoryApp" /> can disable **AutoWiki Cloud Sync**. This is an org-level enterprise control available in the <a href={vars.urls.app}>Factory App</a> under **Settings > Enterprise Controls > AutoWiki**.

When AutoWiki Cloud Sync is **disabled**:

- All AutoWiki API endpoints return 403 for your organization
- The `/wiki` command will not store content in the <ProductName variant="factoryApp" />
- GitHub Wiki sync is also skipped from the Droid CLI
- The AutoWiki page in the <ProductName variant="factoryApp" /> shows no content
