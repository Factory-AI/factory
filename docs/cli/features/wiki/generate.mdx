---
title: Generate AutoWiki
description: Use the /wiki command to generate comprehensive AutoWiki documentation from the Droid CLI.
keywords: ['AutoWiki', 'wiki', '/wiki', 'generate', 'documentation', 'codebase docs', 'slash command', 'upload', 'github wiki sync']
---

The `/wiki` command analyzes your repository and generates structured AutoWiki documentation covering architecture, modules, APIs, and conventions. The resulting documentation is uploaded to Factory cloud and optionally synced to your GitHub wiki tab.
...
<Steps>
  ...
  <Step title="View the result">
    After upload completes, Droid prints a link to your AutoWiki run:
    ```
...
3. **Visual screenshots** -- If the repository has a [QA skill](/guides/skills/automated-qa) installed, Droid launches the application and captures screenshots of web UIs and TUIs to include in AutoWiki documentation. Run `/install-qa` to set one up if your repo doesn't have one yet.
...
<Note>
  AutoWiki generation uses your current branch and working directory state.
  For the most accurate documentation, run `/wiki` from a clean checkout
  of your default branch.
</Note>

<Tip>
  The first AutoWiki generation for a repository can take a while -- Droid needs
...
After uploading to Factory cloud, the Droid CLI automatically syncs the generated AutoWiki documentation to GitHub's built-in wiki tab for GitHub-hosted repositories. The sync:
...
- Generates `Home.md` from your AutoWiki root page
...
If the GitHub wiki isn't initialized, the Droid CLI prints a message with a link to create the first page and skips the sync without failing.

## Versioning

Each AutoWiki generation creates a new **AutoWiki run** with metadata including:
...
You can browse previous versions from the [AutoWiki viewer](/cli/features/wiki/web-viewer) using the version dropdown.

## See also

- [Set up AutoWiki refresh](/cli/features/wiki/auto-refresh) -- Keep AutoWiki current with a CI action
- [Browse AutoWiki](/cli/features/wiki/web-viewer) -- Read, search, and export from the Factory App
