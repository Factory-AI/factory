...
description: Use /install-wiki to set up a CI action that regenerates a Wiki on every push to the default branch.
keywords: ['autowiki', 'wiki', '/install-wiki', 'auto-refresh', 'github actions', 'gitlab ci', 'ci', 'recurring', 'workflow', 'automation']
---
...
<Steps>
  ...
  <Step title="Add the Factory API key">
    ...
    Generate a key in the <a href={`${vars.urls.app}/settings/api-keys`}>Factory API keys settings</a>.
  </Step>
...
- A <ProductName variant="factory" /> API key -- generate one in the <a href={`${vars.urls.app}/settings/api-keys`}>Factory API keys settings</a>
- Permission to add secrets and merge PRs in the target repository

## Refreshing from the Factory App

You can also set up recurring refreshes from the <ProductName variant="factoryApp" />:

1. Go to <a href={`${vars.urls.app}/wiki`}>app.factory.ai/wiki</a>
...
For batch operations across multiple repositories, the <ProductName variant="factoryApp" /> lets you select several repos and run the setup on a Droid Computer in a single operation.

See [AutoWiki in the Factory App](/cli/features/wiki/web-viewer) for more on the <ProductName variant="factoryApp" /> interface.
...
<AccordionGroup>
  <Accordion title="Workflow fails with authentication error">
    ...
    **Settings > Secrets and variables > Actions**. Generate a new key at
    <a href={`${vars.urls.app}/settings/api-keys`}>Factory API keys settings</a>
    if needed.
  </Accordion>

  <Accordion title="AutoWiki not updating after push">
  ...
  <Accordion title="GitHub Wiki not syncing">
    ...
    [AutoWiki Cloud Sync](/cli/features/wiki/overview#autowiki-cloud-sync).
  </Accordion>
</AccordionGroup>
...
- [Browse Wikis](/cli/features/wiki/web-viewer) -- Read and search Wikis from the Factory App
- [Code review automation](/guides/droid-exec/code-review) -- Similar workflow for GitHub App-backed review automation
