...
description: Use /install-wiki to set up a CI action that regenerates AutoWiki documentation on every push to the default branch.
keywords: ['AutoWiki', 'wiki', '/install-wiki', 'auto-refresh', 'github actions', 'gitlab ci', 'CI', 'recurring', 'workflow', 'automation']
---

The `/install-wiki` command creates a CI workflow that automatically regenerates AutoWiki documentation every time code is pushed to the default branch. It detects your CI framework -- GitHub Actions or GitLab CI -- and generates the appropriate configuration. This keeps your documentation in sync with your codebase without manual intervention.
...
<Steps>
  ...
  <Step title="Merge the PR">
    Once the secret is configured, merge the workflow PR. AutoWiki will now
...
- **Run `/wiki`** in headless mode with high autonomy, generating and uploading AutoWiki documentation
...
2. Click **Refresh** on the AutoWiki page
...
See [AutoWiki Viewer](/cli/features/wiki/web-viewer) for more on the Factory App interface.
...
- [Generate AutoWiki](/cli/features/wiki/generate) -- One-time AutoWiki generation with `/wiki`
- [Browse AutoWiki](/cli/features/wiki/web-viewer) -- Read and search from the Factory App
