...
description: Set up Droid to investigate incidents from Slack, run RCA workflows, and keep response work linked to the alert channel.
keywords:
  [
    "incident response",
    "RCA",
...
Incident Response turns production alerts and incident threads into Droid-led RCA investigations.

It helps responders move from alert to investigation faster by automatically starting a linked Droid session with the right context, machine, and instructions.

## Prerequisites

Before setting up Incident Response, make sure you have:

- A connected [Slack integration](/integrations/slack).
- The Factory Slack app invited to the incident channel with `/invite @Factory`.
- A Factory **Manager** or **Owner** role. Channel auto-run settings require elevated org permissions.

<Note>
  Connecting Slack itself may require Slack workspace admin permissions. Configuring Factory channel auto-run requires a Factory Manager or Owner role.
</Note>
...
<Steps>
  <Step title="Open the Slack integration settings">
    In the Factory App, go to **Settings → Integrations → Slack**. Confirm Slack is connected, then open the Slack management view.
  </Step>
...
## Quickstart: set up from Droid CLI
...
2. Factory recognizes the channel and auto-run settings.
3. Factory starts a Droid session using the configured owner, machine, visibility, model, and prompt.
4. Droid receives the Slack message and incident instructions.
5. Droid checks for an `incident-guidelines` runbook on the local device and reuses any matching alert-type guidance before rediscovering tools, auth, repos, and investigation paths.
6. Droid investigates with the configured tools and should invoke the `/incident` workflow when RCA is appropriate.
7. After a confirmed RCA, Droid can save reusable learnings back to `incident-guidelines` so future incidents of the same type start with the known tools, repos, auth methods, and gotchas.
8. Factory posts status and session links back into Slack so responders can follow the investigation.

The generated session gives Droid a strong starting point: the Slack alert text, the channel-specific prompt, access to the selected execution environment, and any matching local incident runbook. From there, Droid can inspect code, query logs, read dashboards through connected tools, ask for clarification when supported, and summarize likely root cause, impact, mitigations, and follow-up actions.

The `incident-guidelines` runbook is stored as a local skill, either in the project at `.factory/skills/incident-guidelines/SKILL.md` for team reuse or in `~/.factory/skills/incident-guidelines/SKILL.md` for personal reuse. It stores concise, reusable mappings for alert types, required tools, interfaces, auth methods, repositories, and generic gotchas; it should not store secrets, account IDs, hardcoded URLs, one-off RCA details, or inline scripts.
