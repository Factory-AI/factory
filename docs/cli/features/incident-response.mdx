...
<Steps>
  ...
  <Step title="Configure MCP servers">
    Use **Configure MCP Servers** to add and authenticate the observability tools and context sources Droid needs to respond to incidents. MCP servers are configured per computer.
  </Step>
...
- A [service account](/enterprise/service-accounts) runs sessions from that account's identity and computers. This is recommended for incident response so the computer and session interactivity are shared.
