...
<Steps>
  ...
  <Step title="Name the automation and select trigger channels">
    Give the automation a name, then under **Triggers**, search for and select the incident channel. Use **Trigger on messages from** to limit which senders start sessions, such as the alert bots that post incidents.
  </Step>

  <Step title="Choose Run as and Run on">
    - **Run as** sets the identity and billing the automation runs under. Keep **Myself** or select a [service account](/enterprise/service-accounts) when incident sessions should run from a shared, preconfigured account.
    - **Run on** sets the computer where Droid investigates. Pick the environment with the right repository checkout, observability tools, and credentials.
  </Step>

  <Step title="Set the prompt and model">
    Enter the instructions injected into every session triggered from the channel and optionally select a model.
  ...
  <Step title="Set session privacy and create">
    Choose whether triggered sessions are **Private** or visible to the organization, review any **Optional Settings**, then click **Create**.
  </Step>
...
The channel picker only shows channels where Factory has been invited. Use **Trigger on messages from** to restrict which senders start sessions, such as your alert bots, so ordinary messages in the channel do not trigger investigations.

### Run as

**Run as** sets the identity and billing the automation runs under.

- **Myself** runs incident sessions as your Factory user, using your connected tools and organization permissions.
- A [service account](/enterprise/service-accounts) runs sessions from that account's identity and computers.

When a service account is selected, the **Run on** picker is limited to computers owned by that service account. Use this for shared incident-response setups where the automation should always run from the same preconfigured account and machine.

If a session fails because the run identity cannot be found or lacks access, edit the automation to use a current user or service account with the right permissions.

### Run on

**Run on** selects the computer where Droid runs the investigation. The computer should have:
...
**Session privacy** controls who can see triggered sessions. Private sessions are visible to the run identity. Organization-visible sessions can be reviewed by other members, which is useful for incident handoff and postmortem review.
...
<AccordionGroup>
  <Accordion title="The Slack channel does not appear in the channel picker">
    Invite Factory to the channel with `/invite @Factory`, then search for the channel again in the automation's trigger settings or re-run `/setup-incident-response`. Private channels must invite Factory explicitly.
  </Accordion>
