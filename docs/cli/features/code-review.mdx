...
## Legacy Review Droid configuration

Review Droid `.droid.yaml` settings were an older configuration surface. Use the current workflow surfaces instead:

- Run `/review` for interactive code reviews that use the rubric above.
- Use [Automated Code Review](/guides/droid-exec/code-review) for pull request automation and GitHub Actions configuration.
- Add repository-specific review criteria with `.factory/skills/review-guidelines/SKILL.md`; see [Custom review guidelines](/guides/droid-exec/code-review#custom-review-guidelines).
