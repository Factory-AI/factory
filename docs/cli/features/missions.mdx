...
### Plan the right level of granularity

Two planning levers have an outsized impact on mission quality, cost, and duration:

- **Features** define worker scope. Breaking work into more features gives each worker a tighter, more focused task, which often improves quality—but it can increase total cost.
- *Milestones** define validation frequency. More milestones trigger validator workers more often, increasing runtime. For simple projects, one milestone is often enough; for longer or complex projects, frequent milestone validation helps keep the foundation stable as work scales.
...
`total runs ≈ #features + #milestones`
