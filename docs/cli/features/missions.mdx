...
Access Factory Missions with the `/missions` command (also available via `/mission`).
...
<CardGroup cols={2}>
  ...
  <Card title="Your Config Carries Over" icon="gear">
    MCP integrations, skills, hooks, and custom droids all work inside Factory Missions.
  </Card>
</CardGroup>
...
<Steps>
  <Step title="Enter Factory Missions">
  ...
  <Step title="Enter Mission Control">
    Once the plan is approved, Droid enters Mission Control -- the Factory Missions orchestration view that manages execution of the plan. You can monitor progress, see which features are being worked on, and intervene when needed.
  </Step>
</Steps>

## The planning phase matters most

The biggest value we have found in Factory Missions is in the planning phase. Getting the upfront plan right -- the features, the ordering, the milestones, the skills involved -- is what determines whether the execution succeeds. Droid will push back, ask questions, and iterate with you until the plan is solid.
...
## What Factory Missions are good for

We have built and tested Factory Missions across a range of work:
...
Factory Missions are not fire-and-forget. The orchestrator is an agent, and you can talk to it. The most effective way to use Factory Missions is to treat yourself as the project manager -- monitoring progress, unblocking workers, and redirecting when the plan needs to change.
...
Factory Missions inherit your existing Droid configuration:
...
Factory Missions are early. We are shipping this as a research preview because there are fundamental questions we are still working through:
...
We want your feedback on these. Use Factory Missions, push the workflow hard, and tell us what works and what does not.
...
- [Custom Droids](/cli/configuration/custom-droids) -- Build specialized subagents that Factory Missions can use
- [Skills](/cli/configuration/skills) -- Create and manage skills that Factory Missions can leverage
