...
If you're setting up BYOM from the desktop experience, open **Settings → Droid Computers** and enable **Remote Access** for that computer so Droid can use it from the web or other devices. This will automatically connect your local machine to our relay service to be reachable from other devices. The connection will remain active for as long as the desktop app is running.
...
After registering the machine, open **Settings → Droid Computers** and enable **Remote Access** for that computer so Droid can use it from the web or other devices.

Then start the daemon so Factory can connect to the machine remotely:
