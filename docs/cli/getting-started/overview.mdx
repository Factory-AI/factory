...
You're now connected to <ProductName variant="factory" />'s development agent from your terminal. [Try the 5-minute quickstart →](/cli/getting-started/quickstart)
...
- **Missions**: Launch multi-agent projects where an orchestrator coordinates specialized worker droids to plan, implement, and validate larger features. Start one with `/missions` or run in headless mode with `droid exec --mission`. [Learn more →](/cli/features/missions)
...
- **Exec Mode**: Run droid in headless mode for scripts, CI/CD pipelines, and automation. Use `droid exec` with structured input and output formats. [Learn more →](/cli/droid-exec/overview)
