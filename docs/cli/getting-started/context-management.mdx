...
**Why not just re-summarize everything?** Re-summarizing the entire conversation creates redundant work, linear cost growth, and quality degradation. Factory uses incremental compression instead - only new content summarizes, not the entire history repeatedly.
...
## Recommendations for extended sessions
...
- To-do list moves forward ✅
- Features work as expected ✅
- Code quality stays high ✅

Progress indicates context management works correctly.

## Example: OAuth authentication handoff

Factory's compression mirrors how engineers hand off work to teammates. Working on OAuth authentication, the agent retains:
...
- ✅ User model created with OAuth fields
- 🔄 GitHub OAuth in progress
- ⏳ Session middleware pending
- ⏳ Protected route examples pending
...
Compressed context preserves decisions and outcomes like an engineering handoff. Exploratory questions, failed attempts, and resolved debugging compress away.
...
<AccordionGroup>
  ...
  <Accordion title="V3: Selective persistence (current approach)">
    ...
    - Conversation history → summarized decisions
    - Exploratory work → outcomes only
    - Resolved issues → lessons learned
    - Multiple iterations → final approach

    This hybrid approach delivers the best of both worlds: **comprehensive context retention with optimal performance**.
  </Accordion>

  <Accordion title="V4: Proactive compression (planned)">
    ...
    **Planned capabilities:**
    - **Natural breakpoint detection** - Agents recognize when to compress completed work phases
    - **Structured memory** - Persistent artifacts like task lists and decision logs (currently implemented)
    - **Sub-agent architectures** - Retrieval agents gather inputs, parent agents keep only results
    - **Intelligent curation** - Agents compress based on relevance, not age

    Compression will shift from "when forced" to "when optimal."
  </Accordion>
</AccordionGroup>
