...
<Info>
ResponseError is Factory's internal error handling class used to throw HTTP-style errors with specific status codes (e.g., ResponseError400BadRequest, ResponseError401Unauthorized...). These error classes are used throughout the codebase to handle API errors. They used to contain generic messages like "Invalid request" or "Not found" that didn't provide enough context for users/developers to understand what went wrong or how to fix it.
</Info>
