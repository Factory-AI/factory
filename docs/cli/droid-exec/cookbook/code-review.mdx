      ...
      - name: Prepare context
        ...
        with:
          script: |
            ...
            // Fetch changed files with line position info
...
The agent should focus on real bugs, not style issues:
- **Find**: Logic errors, security vulnerabilities, performance problems
- **Skip**: Code style, naming conventions, architectural decisions
- **Format**: Clear, actionable feedback with suggested fixes
...
```yaml
      - name: Generate inline review
        ...
        timeout-minutes: 10
        run: |
          # Copy context files for droid to access
          cp .git/pr_review/*.json .git/pr_review/diff.txt ./ 2>/dev/null || true
          ...
          - Max 10 comments per review

          Output: Array of comment objects or empty array if no issues
          EOF

          # Run droid exec with medium autonomy (can create files)
          droid exec --output-format debug --auto medium --model gpt-5-codex -f prompt.txt
...
```yaml
      - name: Supersede outdated comments
        if: always()
        uses: actions/github-script@v7
        with:
          script: |
            ...
            // Find and update old "no issues" comments
            const prNumber = context.payload.pull_request.number;
            ...
            for (const c of existingComments) {
              ...
              if (body.includes('no issues') || body === 'lgtm') {
        ...
        with:
          script: |
            ...
            if (!Array.isArray(comments) || comments.length === 0) {
              // Only post "no issues" if we haven't already
              ...
              const hasNoIssues = existing.some(c => 
                /no issues/i.test(c.body || '')
              );
            ...
            await github.rest.pulls.createReview({
              ...
              pull_number: prNumber,
              event: 'COMMENT',
              body: 'Automated review completed with inline comments.',
              comments: comments
            });
```
...
function processData(data) {
  ...
  if (data = null) {  // Assignment instead of comparison
    return;
  }
}
```
...
Modify the prompt to focus on specific issue types relevant to your codebase:

```yaml
# For a React project
Focus on:
- React hooks rules violations
- State mutation in components
...
The `--auto` flag controls what Droid can do:
- `--auto low`: Read-only analysis (safest, but can't create output files)
- `--auto medium`: Can create files (default for this workflow)
- `--auto high`: Can execute commands (not needed for reviews)

### Use different models

Specify different models for cost/performance tradeoffs:

```bash
# Faster, cheaper model
droid exec --model claude-haiku-20240307 -f prompt.txt
...
Add delays between PR events or use the concurrency group to prevent hitting API rate limits.
