...
### Build on raw JSON-RPC

For custom integrations, you can run Droid as a long-lived subprocess and speak JSON-RPC over stdin/stdout:

```bash
droid exec --input-format stream-jsonrpc --output-format stream-jsonrpc --auto low
```

Each stdin line is one JSON-RPC request. Each stdout line is a JSON-RPC response, server request, or notification. A minimal client typically:
...
- Implements timeouts, interrupts, process cleanup, and session persistence

You can build on top of raw stdin/stdout to create:

- Web or desktop chat UIs that stream Droid responses into your own interface
- CI job runners that execute Droid tasks and surface progress in build logs
- Workflow engines that enqueue prompts, resume sessions, and persist results
- Policy layers that approve, deny, or audit tool permission requests
- Observability pipelines that capture tool use, token usage, errors, and turn completion
- Custom session managers for multi-turn, resumable, or forked conversations
