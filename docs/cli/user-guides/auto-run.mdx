...
- **Subagents** – Task-launched subagents inherit the parent session's Autonomy Level by default, or use the **Subagent autonomy level** setting (`off`/`low`/`medium`/`high`) when configured, always clamped to the organization Maximum. In Spec Mode they are read-only. Organization and Droid tool policy can still restrict them. See [Subagents](/cli/configuration/custom-droids).
- **Factory Missions** – Missions orchestration requires High autonomy or `--skip-permissions-unsafe` (unsafe: skips all permission checks; use only in isolated sandboxes), and admins can restrict who can start Missions. See [Factory Missions](/features/missions/overview).
