---
title: "Deploy Factory Droid on a VPS Server"
description: "Learn how to set up Factory CLI's droid agent on a VPS for remote access and headless automation"
---
...
- Set up Factory CLI's droid agent on a Virtual Private Server (VPS) for remote access from anywhere
...
- **Factory CLI installed locally** - Follow the [installation guide](https://docs.factory.ai/factory-cli/installation)
...
**Problem statement**: Running AI agents locally limits you to when you're at your computer. Many automation tasks need to run continuously, respond to triggers, or be accessible from anywhere - whether you're on a mobile device, traveling, or working from a different machine.

**Concrete pain examples**:
- You want to monitor and manage production servers but can't SSH in from your phone effectively
- You need to run scheduled automation tasks that require AI assistance 24/7
- You want to debug issues on live environments but don't have access to your development machine
- Manual server administration tasks consume hours of your time that could be automated

**How droid on a VPS solves this**: By deploying Factory's droid agent on a cloud server, you get:
- **Always-available AI assistant** - Access droid from any device, anywhere with internet
- **Headless automation** - Run `droid exec` commands programmatically for building custom agents and workflows
- **Production debugging** - Use AI to explore and troubleshoot your live environment directly
- **System administration** - Let droid handle configuration, setup, and maintenance tasks

**Technical foundation**: Factory's droid runs on any Linux environment with Python support. When deployed on a VPS, it becomes a persistent service you can interact with through SSH from any client.
...
### Step 5: Connect and install Factory CLI on the VPS
...
curl -fsSL https://raw.githubusercontent.com/factory-ai/factory-cli/main/install.sh | bash
...
### Step 6: Authenticate Factory CLI on the VPS
...
<Note>See the full droid exec docs [here](https://docs.factory.ai/factory-cli/droid-exec).</Note>
...
**Why GLM 4.6?** It's an extremely fast model, ideal for quick queries and automation tasks where speed matters more than deep reasoning.
...
Because `droid exec` is scriptable, you can build sophisticated automation:

```bash
#!/bin/bash
# Example: Automated deployment checker

# Check deployment status
STATUS=$(droid exec --model glm-4.6 "Check if my Docker containers are running and report their health")

# If issues found, investigate
if echo "$STATUS" | grep -q "error\|stopped"; then
    droid exec --model glm-4.6 "Diagnose the Docker container issue and suggest fixes"
fi
```

**Use cases for custom agents**:
- Automated monitoring and alerting
- Self-healing infrastructure (detect and fix common issues)
- Intelligent log analysis and error triage
- Dynamic configuration management based on system state
...
4. **Join the community**: Share your use cases and learn from others in the [Factory Discord](https://discord.gg/factory)

For more information:
- [Factory CLI Documentation](https://docs.factory.ai/factory-cli)
- [Droid Exec Reference](https://docs.factory.ai/factory-cli/droid-exec)
- [Custom Droids Guide](https://docs.factory.ai/factory-cli/custom-droids)
