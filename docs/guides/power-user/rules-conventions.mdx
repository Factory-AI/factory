...
<Info>
  **Works with Factory App:** These conventions work identically in both the Droid CLI and the [Factory App](/web/getting-started/overview)—Droid reads the same `.factory/rules/` files regardless of interface.
</Info>
...
<Tip>
  See [Hooks reference](/reference/hooks-reference) for more examples.
</Tip>
