...
2. If commands need secrets, add them in “Environment Variables”. They’ll be available as normal env vars during execution.
3. Submit. Commands run in the repo root with bash strict mode (`set -euo pipefail`). The first failing command stops the build.
4. Keep commands non‑interactive and idempotent. Each command can be up to 2048 characters.
5. Review build logs for lines like `=== Running command X/N: ... ===` if anything fails.
