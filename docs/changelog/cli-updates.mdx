...
<Update label="December 10" rss={{ title: "CLI Updates", description: "MCP tools in droid creation and droid management fixes" }}>
  ...
  * **Add reasoning_effort to stream-json** - System init event now includes reasoning effort level
  ...
  * Fixed blocking droid updates to ensure changes are applied correctly
  * Copy binaries instead of symlinking to allow independent updates
