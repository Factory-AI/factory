...
<Update label="November 19" rss={{ title: "CLI Updates", description: "Background processes, MCP search, and TUI stability improvements" }}>
  `v0.26.8`

  ## New features

  * **Background Processes Support** - Added support for running and managing background processes in the CLI. Includes a new `bg-process` command to list, kill, and cleanup processes, with persistent process state tracking
