...
<Update label="December 5" rss={{ title: "CLI Updates", description: "Readiness report command, ESC key improvements, and input fixes" }}>
  ...
  * **Readiness report slash command** - The `/readiness` command is now enabled by default
  * **ESC key closes expanded tool results** - Press ESC to close the expanded tool result view
