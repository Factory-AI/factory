...
<Update label="October 31" rss={{ title: "CLI Updates", description: "Reasoning field support, Import from Claude, and bug fixes" }}>
  ...
  * **Reasoning field support in streaming responses** - Added support for displaying reasoning fields in chat completion streaming chunks for models that provide extended reasoning (e.g., o1)
  ...
  * Fixed stale S3 feature flag reference in LLM streaming hook
  * Fixed MCP status indicator to only show when MCP servers are actually configured
  * Fixed Gemini tool call ID mapping to ensure proper tool execution tracking
  * Fixed message list flickering by treating text messages as stable checkpoints in the static/dynamic split logic
  * Updated FetchUrl tool prompt for better clarity
</Update>
