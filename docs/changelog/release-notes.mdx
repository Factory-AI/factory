...
<Update label="June 18" rss={{ title: "CLI Updates", description: "Private npm registry auth for plugins" }}>
  `v0.151.0`

## New features

- **Private npm registry auth** - Plugin npm sources now support `authTokenEnvVar` for token-based authentication with private registries (JFrog Artifactory, AWS CodeArtifact, GitHub Packages)
