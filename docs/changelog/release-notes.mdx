---
title: "Release notes"
description: "Recent features and improvements to Droid CLI"
rss: true
---
...
- **Computer reconnection** - Computers now reconnect automatically on activity (Factory App)
- **Clearer daemon error messages** - Improved daemon error messaging in desktop (Factory App)
...
- **Per-user computer controls** - Admins can now toggle managed and bring-your-own-machine (BYOM) computers on a per-user basis (Factory App)
...
- **Working directory after machine change** - The working directory is now restored when the selected machine changes (Factory App)
...
<Update label="May 6" rss={{ title: "CLI Updates", description: "Windows setup script, MCP menu revamp, sticky user messages, AutoWiki run deletion, and stability fixes" }}>
  `v0.119.0`

## New features

- **Windows setup script** - New setup script simplifies installing Droid CLI on Windows
...
- **Delete AutoWiki runs** - You can now delete AutoWiki runs directly from the Factory App (Factory App)

## Improvements

- **Desktop auto-updater UX** - Overhauled the auto-updater experience for a clearer update flow (Factory App)
- **Computer wizard polish** - Streamlined the new computer wizard (Factory App)
...
- **Factory Missions overage error clarity** - Distinguishes Factory Missions overage network errors from generic failures
...
- **Live streaming in the app** - Desktop now streams partial text and thinking updates as a droid works (Factory App)
...
- **/missions reminder in Factory Missions sessions** - The /missions system reminder no longer shows when you're already in a Factory Missions session
...
- **Working directory persistence** - Validated working directory is now persisted and locked until the computer connects (Factory App)
- **Computer details on transient errors** - Computer details now remain visible during transient query errors (Factory App)
- **Desktop updater download** - Desktop updater now downloads the new version only after you click to install (Factory App)
...
- **No-computer state on new session** - The new session page now handles cases when no computers are available (Factory App)
...
- **AutoWiki export validation** - Hardened filename validation when exporting AutoWiki documentation (Factory App)
...
- **BYOM admin toggle** - Organization admins can now enable or disable bring-your-own-machine (BYOM) computers from enterprise settings (Factory App)
...
- **Draggable window during startup** - Desktop window stays draggable while the app finishes loading (Factory App)
...
- **macOS desktop updater** - Fixed the desktop auto-updater on macOS (Factory App)
...
- **Factory Missions model settings** - Factory Missions model settings are now snapshotted at launch so they don't drift mid-run
...
- **Desktop local default** - Desktop now consistently defaults to local connections (Factory App)
...
- **Italian translations** - Droid CLI is now available in Italian
...
- **Factory Missions Rate Limits** - Factory Missions now handle Rate Limit responses gracefully
- **Factory Missions entry overage gating** - Factory Missions entry now respects your organization's overage preference
...
- **Devtools shortcut on desktop** - Re-enabled the devtools keyboard shortcut in production builds (Factory App)
...
- **Factory Missions wake-lock leaks on Linux** - Prevented wake-lock leaks during Factory Missions runs on Linux
...
- **Cloud-synced computer sessions** - Cloud-synced computer sessions now display correctly (Factory App)
...
- **System theming** - Factory App now follows your system theme (Factory App)
- **New Factory App updater** - Improved auto-update flow for the Factory App (Factory App)
...
- **Cron schedules in UTC** - Scheduled Factory Missions runs now evaluate cron expressions in UTC
...
- **Web auth timeout fallback** - Fixed an auth timeout fallback in the web app (Factory App)
- **App UI polish** - Fixed various spacing, color, and chevron issues (Factory App)

</Update>

<Update label="April 23" rss={{ title: "CLI Updates", description: "Fast-mode toggle, Droid Computer improvements, Factory Missions artifact validation, and CLI stability fixes" }}>
  `v0.109.1`
...
- **Factory Missions artifact validation** - Factory Missions artifacts are now schema-validated on create, edit, and apply-patch
...
<Update label="April 22" rss={{ title: "CLI Updates", description: "Consolidated Factory Missions files, compute usage on billing pages, and CLI stability fixes" }}>
  `v0.108.0`

## New features

- **Consolidated Factory Missions files** - Files related to Factory Missions now live together under `~/.factory` for easier discoverability
- **Compute usage on billing pages** - Billing and usage pages now surface compute usage alongside other usage data (Factory App)

## Bug fixes

- **Factory Missions wake-lock on Linux** - Factory Missions wake-lock now also blocks idle and lid-switch suspend so long-running Factory Missions runs aren't interrupted
...
- **Factory App stability** - Improved crash reporting and shutdown handling in the Factory App (Factory App)
- **Onboarding wizard display** - Fixed display issues in the onboarding wizard (Factory App)
- **Computer connection warning** - Suppressed a spurious "Failed to get computer connection state" warning (Factory App)
...
<Update label="April 20" rss={{ title: "CLI Updates", description: "Kimi K2.6 support, simplify built-in skill, AutoWiki GitHub sync, and CLI stability fixes" }}>
  `v0.105.0`
...
- **AutoWiki GitHub sync** - AutoWiki pages can now sync to GitHub, with additional AutoWiki reader fixes
- **Managed computers enterprise control** - New managed computers toggle in enterprise controls (Factory App)
- **Factory Missions access policy** - Enterprise admins can now control access to Factory Missions
- **Session title in browser tab** - Web sessions now use the session title as the page title (Factory App)
- **Revamped computer creation** - Redesigned flow for creating Droid Computers (Factory App)
- **Nested sidebar** - Sidebar now supports nesting along with other polish improvements (Factory App)
- **Computer connection status** - Machine selector now shows the connection status for each computer (Factory App)
...
- **Factory Missions worker permissions** - Factory Missions worker permission requests are now auto-denied
...
- **Stuck "Show more"** - Fixed a stuck "Show more" button after creating a session on a new computer (Factory App)
...
- **Source control warning for Computers** - Warning displayed when creating a Droid Computer without source control configured (Factory App)
...
- **Computer terminal** - Fixed terminal display issues when using Droid Computers (Factory App)
...
- **macOS Spotlight discoverability** - Factory App is now easier to find via macOS Spotlight search (Factory App)
...
- **Diff viewer for Computers** - Added diff viewer support for viewing file changes in Droid Computers (Factory App)
...
- **Pre-session bug reports** - Submit bug reports from the Factory App even when no session is active (Factory App)
...
- **Factory App UI polish** - Various visual refinements across the Factory App (Factory App)

## Bug fixes

- **Duplicate tool call messages** - Fixed duplicate tool call messages and missing updates during tool execution
- **Factory Missions feature counts** - Fixed incorrect feature counts in `/missions`
...
- **Font size consistency** - Fixed font size mismatch in the Computers section (Factory App)
...
- **Custom model validation** - Fixed validation errors when using custom models in Factory Missions workers
...
- **JSON rendering in chat** - JSON data now renders with improved formatting in web and desktop chat (Factory App)
...
- **Factory Missions mode input** - Fixed input field behavior in Factory Missions mode (Factory App)

</Update>

<Update label="April 2" rss={{ title: "CLI Updates", description: "Unified Factory Missions menu, settings and footer redesign, session forking, TUI charts, and SSH clipboard fixes" }}>
  `v0.92.0`

## New features

- **Unified /missions menu** - Redesigned the Factory Missions `/missions` command with model tabs and improved navigation
...
- **Faster Factory App session loading** - Sessions load directly from disk in the Factory App for improved speed (Factory App)
...
<Update label="March 27" rss={{ title: "CLI Updates", description: "Spec mode redesign, Factory Missions worker view, fuzzy model search, and cleaner tool output" }}>
  `v0.89.0`

## New features

- **Factory Missions worker view** - New view for monitoring individual Factory Missions worker progress and output
...
- **Billable Tokens renamed to Factory Standard Credits** - "Billable Tokens" renamed to "Factory Standard Credits" throughout the app UI (Factory App)
- **Auto-updating plugins** - Installed plugins now auto-update on CLI startup

## Bug fixes

- **Factory Missions worker stability** - Permission blocks in Factory Missions workers no longer terminate the session
...
<Update label="March 26" rss={{ title: "CLI Updates", description: "Spec approval handoff, shimmer animations for tool headers, and Factory Missions reliability improvements" }}>
  `v0.87.0`
...
- **Factory Missions worker startup** - Improved Factory Missions worker startup reliability with extended timeout
- **Factory Missions connection stability** - Increased socket connection timeout for more reliable Factory Missions connections
- **Computer session creation** - Fixed errors when creating sessions while the computer is temporarily unavailable (Factory App)
...
- **Factory Missions worker BYOK settings** - Fixed Factory Missions worker settings not persisting correctly when using custom API keys
...
- **Computer connection reliability** - Improved timeouts for initial computer connections (Factory App)
...
- **Session type tags** - Sessions now display type tags for Factory Missions and subagents for easier identification
...
- **Computer info in session tooltip** - Session tooltips now display connected computer details (Factory App)
...
- **Computer install commands** - The Connect to Computer modal now shows curl and Windows install commands (Factory App)
...
- **Computer card navigation** - Fixed click navigation on computer name cards (Factory App)
...
- **Computer selection UI** - Improved visual design for the computer selection interface (Factory App)

## Bug fixes

- **Slash command arguments** - Fixed slash command arguments being lost when submitting via the suggestion menu
- **Factory Missions model selection** - Factory Missions now correctly uses the session model instead of the global settings model
- **Worktree error messages** - Worktree setup failures now show descriptive error messages instead of generic errors
- **Stale Factory Missions warnings** - Fixed stale Factory Missions runs incorrectly triggering concurrent run warnings
...
- **Automations page layout** - Fixed automations page height on desktop (Factory App)
...
- **Bug report performance** - Fixed bug reports hanging when processing large Factory Missions runs
...
- **Code server connection** - Fixed code server connection issues in the Factory App (Factory App)
...
- **Computer connect modal** - Fixed VS Code config display in the computer connect modal (Factory App)
...
- **Faster computer sessions** - Optimistic preconnection reduces wait time when starting computer sessions (Factory App)
- **Working directory validation** - Computer sessions now validate working directory configuration before starting (Factory App)
...
- **Factory Missions reasoning** - Fixed reasoning issues in Factory Missions mode
...
- **Factory App download button** - Fixed download Factory App button incorrectly showing in the Factory App (Factory App)
...
- **`--mission` flag for droid exec** - New `--mission` flag enables non-interactive Factory Missions orchestration via `droid exec`
...
- **MCP tool rendering** - Improved MCP tool result display in the web app (Factory App)
...
- **VS Code auto-installation** - Fixed embedded VS Code auto-installation in the Factory App (Factory App)
- **Factory App stability** - Fixed daemon restart issues in the Factory App (Factory App)
...
- **Payment redirect on desktop** - Fixed payment page redirect handling in the Factory App (Factory App)
- **Character encoding** - Fixed display of malformed UTF characters in messages
- **Factory Missions list formatting** - Fixed formatting of feature and worker list descriptions in Factory Missions
...
<Update label="March 2" rss={{ title: "CLI Updates", description: "/copy command, marketplace management, and Factory Missions worker improvements" }}>
  `v0.66.0`
...
- **Factory App version display** - Factory App now shows the current version number (Factory App)
- **Grouped exec sessions** - Droid exec sessions are now grouped under a dedicated tab for cleaner organization (Factory App)

## Improvements

- **Factory Missions worker reliability** - Hardened Factory Missions worker lifecycle with improved pause and resume handling
...
<Update label="February 27" rss={{ title: "CLI Updates", description: "Diagnostics command, GitHub comments in diff viewer, and Factory Missions mode fixes" }}>
  `v0.65.0`
...
- **Factory Missions mode model selection** - Fixed model selection issues when using Factory Missions mode
- **Factory Missions mode validation** - Validation contracts now update correctly when Factory Missions requirements change
...
<Update label="February 26" rss={{ title: "CLI Updates", description: "Session archiving, inline rename, custom model reasoning effort, and Factory Missions mode improvements" }}>
  `v0.64.0`
...
- **Concurrent Factory Missions warnings** - Warns on Factory Missions entry and confirms concurrent Factory Missions runs
- **Dry-run validation** - New dry-run validation approach during Factory Missions planning
- **/model scope clarity** - Clarified `/model` command scope and added set-as-default action

## Bug fixes

- **Factory Missions mode commands** - Split Factory Missions mode enter and exit into separate commands for improved reliability
...
- **README requirement for Factory Missions** - Factory Missions now require README updates before completion
...
- **Online research in Factory Missions** - Added online research and environment setup during Factory Missions planning
...
- **Factory Missions latency** - Reduced `/enter-mission` latency and improved daemon error UX
- **Spec/autonomy decoupling** - Decoupled spec and autonomy settings and permission outcomes

</Update>

<Update label="February 20" rss={{ title: "CLI Updates", description: "Faster search, Factory Missions model selector, and Factory Missions validation overhaul" }}>
  `v0.60.0`

## New features

- **Factory Missions model selector** - Choose specific models for Factory Missions workers
- **Prevent idle sleep** - CLI now prevents system idle sleep during Factory Missions execution
...
- **Factory Missions validation overhaul** - Complete overhaul of Factory Missions validation for improved reliability
- **Factory Missions worker timeouts** - Improved Factory Missions worker timeout handling
...
<Update label="February 19" rss={{ title: "CLI Updates", description: "Local settings override, environment variable references in custom models, and Factory Missions UX improvements" }}>
  `v0.58.0`
...
- **Factory Missions planning pausing** - Allow pausing during planning phases with in-context resume in Mission Control view
...
- **Factory App connection race** - Fixed daemon connection race condition in the Factory App with polling retry logic (Factory App)
- **Sidebar sessions** - Fixed sessions disappearing from sidebar (Factory App)
- **Open in editor** - Fixed "open in editor" functionality in the Factory App (Factory App)
...
- **Factory App notifications** - Native macOS and Windows notifications when sessions complete, with configurable preferences (Factory App)
- **Factory App native view redesign** - Refreshed sidebar layout with smooth animations, new terminal editor icons, and keyboard shortcut formatting (Factory App)
...
- **Ask User tool** - Ask User tool now available in the web and Factory App surfaces (Factory App)
...
- **Task tool live streaming** - Real-time subagent tool progress display and Execute tool output streaming in web and Factory App surfaces (Factory App)
...
- **Parallel context gathering** - Factory Missions workers now encouraged to read all context files in parallel for faster startup
...
- **PDF & text file uploads** - Attach PDFs and text files to chat in Web/Desktop apps (Factory App)
