...
- **Expand pasted placeholders** - Press Ctrl+Shift+V to expand truncated paste placeholders inline
...
- **Custom droids directory** - Non-directory entries under `~/.factory/droids` are now skipped without spurious errors
- **OAuth provider errors** - 4xx responses from connected OAuth providers are now surfaced as user-facing errors
- **Hook resilience** - Hooks tolerate malformed entries and preserve their original startup source
...
- **`exec` stream-json pipes** - Stream-json writes in `exec` no longer crash when the receiving pipe closes early
- **Create confirmation crash** - Guarded against missing file paths in Create tool confirmation prompts
...
- **Kimi tool ids** - Hyphenated Kimi tool ids are now regenerated correctly
