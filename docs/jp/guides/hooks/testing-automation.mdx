---
title: "テスト自動化"
description: "フックでテスト実行、カバレッジ追跡、テスト結果検証を自動化する"
...
<Steps>
  <Step title="可能な場合はテストを非同期実行">
    Droidを不要にブロックしないようにします:
    ```bash
  ...
  <Step title="カバレッジを設定可能にする">
    ファイルごとに異なるしきい値が必要な場合があります:
    ```bash
  ...
  <Step title="テスト結果をキャッシュする">
    コードが変更されていない場合はテストをスキップします:
    ```bash
