---
title: "GitHubアクション"
description: droid execですぐに使えるGitHub Actionsワークフロー。
keywords: ['github actions', 'ci/cd', 'workflow', 'automation', 'pr review', 'droid exec']
---

# GitHub Actions

**前提条件:** `FACTORY_API_KEY` をリポジトリシークレットに追加してください（Settings → Secrets → Actions）

## 例1: 自動PR レビューと修正

PR を自動的にレビューし、問題を修正して、詳細なフィードバックを投稿します。

<Info>
より簡単に設定するには、ガイド付き手順でFactory GitHub Appを設定する[`/install-github-app`](/cli/features/install-github-app)コマンドを使用してください。
</Info>
...
コードの変更に合わせて、ドキュメントとテストを自動的に同期させます。
      ...
      - name: Create PR if changes exist
        ...
        run: |
          ...
          if [ -n "$(git status --porcelain)" ]; then
            ...
            [ -f docs-updates.md ] && PR_BODY="${PR_BODY}### ドキュメント\n$(cat docs-updates.md)\n\n"
