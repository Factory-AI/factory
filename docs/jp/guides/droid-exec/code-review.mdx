...
<div style={{ display: 'flex', gap: '1rem', flexWrap: 'wrap' }}>
  <div style={{ flex: '1', minWidth: '300px' }}>
    <img src="./code-review-picture-1.png" alt="見つかった問題を含むコードレビューサマリーを投稿するFactory Droidボット" />
  </div>
  <div style={{ flex: '1', minWidth: '300px' }}>
    <img src="./code-review-picture-2.png" alt="特定の行にインラインコードレビューコメントを投稿するFactory Droidボット" />
  </div>
</div>

## セットアップ

GitHubまたはGitLabの自動コードレビューを設定するには、`/install-code-review`コマンドを使用します：```bash
droid
> /install-code-review
```ガイド付きフローでは以下を行います：
...
GitHub専用のセットアップには `/install-github-app` も使用できます。詳細なセットアップ手順については、[GitHub Appインストールガイド](/jp/cli/features/install-github-app)をご覧ください。
...
- **`shallow`** — 表面的な問題をカバーする、より高速でコスト効率の良いレビュー。大量リポジトリ、下書きPR、または支出を監視するチームに適しています。```yaml
with:
...
ワークフローに `automatic_security_review: true` を追加します。`automatic_review` と `automatic_security_review` の両方が有効な場合、セキュリティレビューアーはコードレビューパス中に並列サブエージェントとして実行されます：```yaml
with:
  automatic_review: true
...
ワークフローにcronトリガーを追加して、定期的なリポジトリ全体のスキャンをスケジュール実行：```yaml
name: Security Scan
on:
  schedule:
...
リポジトリ固有のレビューガイドラインを追加するには、リポジトリ内に`.factory/skills/review-guidelines/SKILL.md`ファイルを作成します：```markdown
...
特定のケースでレビューをスキップするための条件を追加する：```yaml
jobs:
  code-review:
...
- [GitHub Appインストール](/jp/cli/features/install-github-app) - `/install-github-app`の完全セットアップガイド
