...
- **サブエージェント** – Task で起動されたサブエージェントは、デフォルトでは親セッションの自律レベルを継承します。設定されている場合は **Subagent autonomy level** 設定（`inherit` / `off` / `low` / `medium` / `high`）が使われますが、常に組織の Maximum Autonomy Level に制限されます。Spec Mode では読み取り専用です。組織と Droid のツールポリシーでさらに制限される場合があります。詳細は [サブエージェント](/jp/cli/configuration/custom-droids) を参照してください。
