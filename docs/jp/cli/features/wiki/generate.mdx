---
title: Wikiを生成する
description: /wiki コマンドを使って、CLI から包括的なコードベースドキュメントを生成します。
keywords: ['wiki', '/wiki', 'generate', 'documentation', 'codebase docs', 'slash command', 'upload', 'github wiki sync']
---

`/wiki` コマンドはリポジトリを解析し、アーキテクチャ、モジュール、API、規約をカバーする構造化ドキュメントを生成します。生成された wiki は Factory cloud にアップロードされ、必要に応じて GitHub wiki タブにも同期されます。
...
<Steps>
  ...
  <Step title="wiki コマンドを実行する">
    ...
    > /wiki
    ```
    Droid はコードベースを解析し、プロジェクトディレクトリ内のローカル `droid-wiki/` フォルダに Markdown ファイルを生成します。生成が完了すると、内容を Factory cloud にアップロードします。
  </Step>
  <Step title="結果を確認する">
...
4. **Factory cloud へアップロード** -- ページと画像を Factory の API に送信します
...
Factory cloud へのアップロード後、CLI は GitHub でホストされているリポジトリについて、生成された wiki を GitHub の組み込み wiki タブへ自動同期します。この同期では次のことを行います：
...
- 組織で [Wiki Cloud Sync](/jp/cli/features/wiki/overview#wiki-cloud-sync) が無効化されていないこと
...
過去のバージョンは、[Webビューア](/jp/cli/features/wiki/web-viewer) のバージョンドロップダウンから参照できます。

## 関連情報

- [自動更新を設定する](/jp/cli/features/wiki/auto-refresh) -- CI アクションで wiki を最新の状態に保つ
- [Wikiを閲覧する](/jp/cli/features/wiki/web-viewer) -- Web から閲覧、検索、エクスポートを行う
