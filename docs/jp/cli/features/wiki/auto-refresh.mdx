---
title: 自動更新
description: /install-wiki を使って、デフォルトブランチへの push ごとに wiki を再生成する CI アクションを設定します。
...
複数のリポジトリをまとめて操作する場合、Web UI では複数の repo を選択し、1 回の操作で Droid Computer 上でセットアップを実行できます。

Web インターフェースの詳細は [Webビューア](/jp/cli/features/wiki/web-viewer) を参照してください。
...
<AccordionGroup>
  ...
  <Accordion title="push 後に wiki が更新されない">
  ...
  <Accordion title="GitHub wiki が同期されない">
    ...
    組織で [Wiki Cloud Sync](/jp/cli/features/wiki/overview#wiki-cloud-sync) が無効化されていないことも確認してください。
  </Accordion>
</AccordionGroup>

## 関連情報

- [Wikiを生成する](/jp/cli/features/wiki/generate) -- `/wiki` を使った一度きりの wiki 生成
- [Wikiを閲覧する](/jp/cli/features/wiki/web-viewer) -- Web から閲覧と検索を行う
- [GitHub App のインストール](/cli/features/install-github-app) -- コードレビュー自動化向けの類似ワークフロー
