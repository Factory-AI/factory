...
Factory 提供の組み込みスキルを無効化するには、セッションの初期化または読み込み時に `disableBuiltinSkills` を設定します：
...
stream JSON-RPC モードでは、`--disable-builtin-skills` CLI フラグの代わりにリクエストフィールドを使用します。TypeScript SDK でも、`createSession({ disableBuiltinSkills: true })` と `resumeSession(sessionId, { disableBuiltinSkills: true })` を通じて同じオプションを利用できます。
...
インタラクティブまたは非インタラクティブなセッションで、Droid CLI の外部から提供されたスキルだけを公開したい場合は、`--disable-builtin-skills` を使用します：
...
これにより、Factory 提供の組み込みスキルは無効化されますが、ユーザー、プロジェクト、プラグイン、自動化、mission、runtime、および動的に提供されるスキルは保持されます。インタラクティブ起動では、この制限はセッションの作成、再開、切り替え後も有効なままです。また、subagent や mission worker を含む子セッションにも継承されます。

<Note>
  このフラグは Agent Client Protocol (ACP) モードではサポートされません。通常の `droid` または `droid exec` で使用するか、stream JSON-RPC モードの `droid.initialize_session` および `droid.load_session` リクエストで `disableBuiltinSkills` を設定してください。
</Note>
