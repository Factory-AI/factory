...
カスタム統合では、Droidを長時間実行されるサブプロセスとして起動し、stdin/stdout経由でJSON-RPCをやり取りできます：

```bash
droid exec --input-format stream-jsonrpc --output-format stream-jsonrpc --auto low
```

stdinの各行は1つのJSON-RPCリクエストです。stdoutの各行はJSON-RPCレスポンス、サーバーからのリクエスト、または通知です。最小限のクライアントは通常、以下を行います：
...
- タイムアウト、中断、プロセスのクリーンアップ、セッション永続化を実装する

生のstdin/stdoutの上に、以下のようなものを構築できます：

- Droidのレスポンスを独自UIにストリーミングするWebまたはデスクトップのチャットUI
- Droidタスクを実行し、進捗をビルドログに表示するCIジョブランナー
- プロンプトをキューに入れ、セッションを再開し、結果を永続化するワークフローエンジン
- ツール権限リクエストを承認、拒否、監査するポリシーレイヤー
- ツール使用、トークン使用量、エラー、ターン完了を収集するオブザーバビリティパイプライン
- マルチターン、再開可能、またはフォーク可能な会話のためのカスタムセッションマネージャー
