...
| `apiKey` | `string` | | プロバイダー用のAPIキー。オプション — 認証不要のエンドポイント（ローカルモデルや認証が不要なゲートウェイなど）では省略できます。`settings.json`/`settings.local.json`での`${VAR_NAME}`をサポートします（例：`${PROVIDER_API_KEY}`は`PROVIDER_API_KEY`環境変数を使用します）。 |
