...
<Note>
**Skills are now user-invokable via slash commands.** You can create skills in __PRESERVE_7__ that work as both slash commands (user-invoked) and Droid-invocable capabilities. Your existing __PRESERVE_8__ files continue to work unchanged. For new commands, consider using [skills](/cli/configuration/skills) instead – they offer additional features like supporting files and [invocation control](/cli/configuration/skills#control-who-invokes-a-skill).
</Note>
...
<Note>
  Positional placeholders like __PRESERVE_26__ or __PRESERVE_27__ are not supported yet—use
  __PRESERVE_28__ and parse inside the prompt if you need structured input.
</Note>
