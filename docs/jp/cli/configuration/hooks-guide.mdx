...
<Tip>
  Include `ApplyPatch` in file-edit matchers when you want hooks to run for patch-based edits. Hooks that read `tool_input.file_path` should handle an empty value because patch tools can modify multiple files.
</Tip>
