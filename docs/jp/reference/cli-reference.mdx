...
#### `droid computer port-forward` のフラグ

マッピングは kubectl 形式です: `LOCAL:REMOTE`（例: `8080:80`）、単一の `REMOTE`（両側で同じポート）、または `:REMOTE`（空いているローカルポートをランダムに使用）。

| フラグ             | 説明                                        | 例                                                          |
| :---------------- | :----------------------------------------- | :--------------------------------------------------------- |
| `--address <addr>` | バインドするローカルアドレス（デフォルト: `127.0.0.1`） | `droid computer port-forward laptop 8080:80 --address 0.0.0.0` |
| `--debug`         | デバッグ出力を有効化                            | `droid computer port-forward laptop 8080:80 --debug`      |
