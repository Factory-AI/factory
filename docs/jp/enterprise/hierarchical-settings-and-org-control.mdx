...
<a id="system-managed-settings-file"></a>
...
<ResponseField name="builtInToolAutonomyOverrides" type="object">
  組み込みネットワークツールを確認プロンプトなしで実行するために必要な、ツールごとの[自律レベル](/jp/cli/user-guides/auto-run)。組み込みツール名を `low`、`medium`、`high` にマッピングします。ツールは、セッションの自律レベルが設定された要件以上の場合にのみ自動実行され、それ以外の場合は Droid が承認を求めます。エントリのないツールはデフォルトで `low` になります。組織管理設定からのみ適用されるため、ユーザーおよびプロジェクト設定で弱めることはできません。Enterprise Controls の **Built-in Tool Autonomy** で設定します。
  <Expandable title="properties">
    <ResponseField name="WebSearch" type="string">
      組み込みの Web Search ツールを実行するために必要な自律レベル。`low`、`medium`、`high` のいずれか。デフォルトは `low`。
    </ResponseField>
    <ResponseField name="FetchUrl" type="string">
      組み込みの Fetch URL ツールを実行するために必要な自律レベル。`low`、`medium`、`high` のいずれか。デフォルトは `low`。
    </ResponseField>
  </Expandable>
</ResponseField>

<ResponseField name="subagentAutonomyLevel" type="string">
  Task ツールによって起動されるサブエージェントに適用される自律レベル。`inherit`、`off`、`low`、`medium`、`high` のいずれか。デフォルトは `inherit` で、親セッションの自律レベルにフォールバックします。明示的なレベルは `maxAutonomyLevel` によって制限されるため、エンタープライズの上限を超えることはありません。
</ResponseField>
...
<ResponseField name="disableWeeklyUsageSummary" type="boolean">
  月間クレジット上限の80%以上に達したユーザーと有効なサービスアカウントを一覧にした週次メールを、組織のマネージャーおよびオーナーへ送信するかどうかを制御します。未設定または `false` の場合は送信され、`true` に設定すると停止します。
</ResponseField>
...
<ResponseField name="allowManagedHooksOnly" type="boolean">
  `true` の場合、組織管理フックと、組織が有効にしたプラグイン由来のフックのみが読み込まれます。ユーザーレベルおよびプロジェクトレベルで定義されたフックは無視されます。デフォルトは `false` で、すべてのレベルのフックが通常どおりマージされます。[組織管理フック](/jp/reference/hooks-reference#org-managed-hooks) を参照してください。
</ResponseField>

<ResponseField name="hooks" type="object">
  [フックイベント](/jp/reference/hooks-reference#hook-events) をキーとする組織管理フック定義。組織が定義したフックは常に読み込まれ、下位レベルで削除することはできません。フックの完全なセマンティクスについては [フックリファレンス](/jp/reference/hooks-reference) を参照してください。
  <Expandable title="properties">
    <ResponseField name="<EventName>" type="object[]">
      フックイベントごとに1つのキー: `PreToolUse`、`PostToolUse`、`Notification`、`UserPromptSubmit`、`Stop`、`SubagentStop`、`PreCompact`、`SessionStart`、`SessionEnd`。各値はマッチャーグループの配列です。不明なイベントキーは許容されますが、読み込み時に警告が表示されます。
      <Expandable title="properties">
        <ResponseField name="matcher" type="string">
          照合するツール名のパターン（`PreToolUse` と `PostToolUse` のみに適用）。
        </ResponseField>
        <ResponseField name="commandRegex" type="string">
          コマンドに対して照合される任意の正規表現。
        </ResponseField>
        <ResponseField name="hooks" type="object[]" required>
          マッチャーが一致したときに実行するコマンド。
          <Expandable title="properties">
...
<ResponseField name="customModels" type="object[]">
  ...
  <Expandable title="properties">
    ...
    <ResponseField name="apiKey" type="string">
      プロバイダーとの認証用 API キー。オプション — 認証不要のエンドポイント（ローカルモデルや認証が不要なゲートウェイなど）では省略できます。`settings.json`/`settings.local.json` で `${VAR_NAME}` をサポートします（例：`${PROVIDER_API_KEY}` は `PROVIDER_API_KEY` 環境変数を使用します）。

      <Warning>
        **静的 API キーは新規の組織管理モデルではブロックされます。** 管理設定エンドポイントは、静的 `apiKey` 値を含む新規カスタムモデルを 400 エラーで拒否します。既存の静的キーエントリは保持およびローテーション可能ですが、新規エントリには以下のいずれかを使用してください：

        - **`apiKeyHelper`** — リクエスト時に短期認証情報を生成するシェルコマンド（ゲートウェイやトークン発行システムに推奨）。
        - **環境変数参照** — `apiKey` を `${VAR_NAME}` の全体参照に設定（例：`"apiKey": "${INTERNAL_MODEL_API_KEY}"`）。実行時にマシン環境から解決されます。
        - **システム管理 (MDM) 設定ファイル** — 静的キーが必要なデプロイメントでは、IT/MDM が各マシンに配置する[システム管理設定ファイル](#システム管理-settingsjson-ファイル)を使用します。これにより管理設定エンドポイントが完全にバイパスされます。
      </Warning>
    </ResponseField>
    <ResponseField name="apiKeyHelper" type="string">
      プロバイダー/ゲートウェイトークンを標準出力に出力するシェルコマンド。リクエスト時に解決され、静的 `apiKey` の代わりに使用されます。組織管理（信頼済み）設定からのみ適用され、ユーザー/プロジェクト/フォルダソースからは除外されます。
    </ResponseField>
    <ResponseField name="apiKeyHelperTtlMs" type="number">
      `apiKeyHelper` 出力の更新間隔（ミリ秒）。期限切れまでキャッシュされたトークンが再利用され、その後ヘルパーコマンドが再実行されます。
    </ResponseField>
    <ResponseField name="authMode" type="string">
      設定された認証情報の認証モード。`"bearer"` は Anthropic SDK の `authToken` パスを使用し、省略または `"provider-default"` はネイティブプロバイダー認証を維持します。
    </ResponseField>
...
| **オープンソース** | `glm-5`, `glm-5.1`, `glm-5.2`, `kimi-k2.5`, `kimi-k2.6`, `minimax-m2.7` |
...
- **Hooks Policy** (`allowManagedHooksOnly`, `hooks`) – 組織管理者が組織管理フックを定義でき、「管理/プラグインフックのみ」を有効にすると、すべてのユーザーレベルおよびプロジェクトレベルのフックが解決時に破棄されます。組織管理フックは権威があり、メンバーがそれらを削除したり独自のフックを再導入したりすることはできません。[組織管理フック](/jp/reference/hooks-reference#org-managed-hooks) を参照してください。
- **Built-in Tool Autonomy** (`builtInToolAutonomyOverrides`) – 組み込みネットワークツール（Web Search、Fetch URL）を確認プロンプトなしで実行するために必要な[自律レベル](/jp/cli/user-guides/auto-run)を設定します。エントリのないツールはデフォルトで `low` になります。組織設定からのみ適用されるため、ユーザーおよびプロジェクトで弱めることはできません。
