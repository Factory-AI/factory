...
<ResponseField name="builtInToolAutonomyOverrides" type="object">
  組み込みのネットワークツールを確認プロンプトなしで実行するために必要な、ツールごとの [自律レベル](/jp/cli/user-guides/auto-run)。組み込みツール名から `low`、`medium`、`high` へのマップです。ツールは、セッションの自律レベルが設定された要件以上の場合にのみ自動実行され、それ以外では Droid が承認を求めます。エントリがないツールは `low` がデフォルトです。組織管理設定からのみ強制されるため、ユーザー設定やプロジェクト設定でこれを緩和することはできません。Enterprise Controls の **Built-in Tool Autonomy** で設定します。
  <Expandable title="プロパティ">
    <ResponseField name="WebSearch" type="string">
      組み込みの Web Search ツールを実行するために必要な自律レベル。`low`、`medium`、`high` のいずれか。デフォルトは `low` です。
    </ResponseField>
    <ResponseField name="FetchUrl" type="string">
      組み込みの Fetch URL ツールを実行するために必要な自律レベル。`low`、`medium`、`high` のいずれか。デフォルトは `low` です。
    </ResponseField>
  </Expandable>
</ResponseField>
...
<ResponseField name="subagentModelSettings" type="object">
  [サブエージェント](/jp/cli/configuration/custom-droids) 向けの複雑さ → モデルのルーティングです。サブエージェントの droid が `model: inherit` を使い、親が complexity ティアを渡したときに適用されます。
  <Expandable title="プロパティ">
    <ResponseField name="lightModel" type="string">
      `light` complexity タスクで使用するモデル。モデル識別子を指定するか、省略して起動元セッションのモデルを継承します。
    </ResponseField>
    <ResponseField name="lightReasoningEffort" type="string">
      light tier の reasoning effort。`reasoningEffort` と同じ値を使用します。
    </ResponseField>
    <ResponseField name="mediumModel" type="string">
      `medium` complexity タスクで使用するモデル。モデル識別子を指定するか、省略して起動元セッションのモデルを継承します。
    </ResponseField>
    <ResponseField name="mediumReasoningEffort" type="string">
      medium tier の reasoning effort。`reasoningEffort` と同じ値を使用します。
    </ResponseField>
    <ResponseField name="heavyModel" type="string">
      `heavy` complexity タスクで使用するモデル。モデル識別子を指定するか、省略して起動元セッションのモデルを継承します。
    </ResponseField>
    <ResponseField name="heavyReasoningEffort" type="string">
      heavy tier の reasoning effort。`reasoningEffort` と同じ値を使用します。
    </ResponseField>
  </Expandable>
</ResponseField>
...
<ResponseField name="disableWeeklyUsageSummary" type="boolean">
  月間クレジット上限の80%以上に達したユーザーと有効なサービスアカウントを一覧にした週次メールを、組織のマネージャーおよびオーナーへ送信するかどうかを制御します。この設定を省略するか `false` にするとサマリーが送信され、停止するには `true` に設定します。
</ResponseField>
...
- `subagentModelSettings.*`（tier ごとのモデルと reasoning effort）
...
- **Built-in Tool Autonomy** (`builtInToolAutonomyOverrides`) – 組み込みのネットワークツール（Web Search、Fetch URL）を確認プロンプトなしで実行するために必要な [自律レベル](/jp/cli/user-guides/auto-run) を設定します。省略されたツールのデフォルトは `low` です。これは組織設定からのみ強制されるため、ユーザーやプロジェクトで緩和することはできません。
