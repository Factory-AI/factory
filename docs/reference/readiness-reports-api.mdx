...
The Readiness Reports API provides programmatic access to your organization's agent readiness evaluation data in <ProductName variant="factory" />.

---

## Authentication

All requests require a <ProductName variant="factory" /> API key in the `Authorization` header:

```bash
Authorization: Bearer fk-your-api-key
```

Generate API keys in the <a href={`${vars.urls.app}/settings/api-keys`}>Factory API keys settings</a>.

---

## Base URL

<code>{vars.urls.app}</code>
