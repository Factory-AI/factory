...
#### Quick start: Adding servers from the registry

The easiest way to get started with MCP is using the built-in registry. Type `/mcp` and select **"Add from Registry"** to browse pre-configured servers:
...
| ... | 40+ servers available |

Select a server, authenticate if required, and you're ready to go. Most HTTP servers support OAuth—just follow the browser prompt.
...
For scripting and automation, use CLI commands instead of the interactive UI:
...
droid mcp add <name> <url> --type http --header "Authorization: Bearer xxx"

# Add stdio-based server (default type)
droid mcp add <name> "<command>"

# Add stdio server with environment variables
droid mcp add <name> "<command>" --env API_KEY=xxx --env DEBUG=true
...
| :----- | :---------- | :--------- |
| `--type <type>` | Server type: `http` or `stdio` (default: `stdio`) | Both |
| `--env <KEY=VALUE>` | Environment variable (can be repeated) | stdio only |
| `--header <KEY: VALUE>` | HTTP header (can be repeated) | http only |
...
| :---- | :------- | :------ |
| **User** | `~/.droid/mcp.json` | Your personal servers, available in all projects |
| **Project** | `.factory/mcp.json` | Shared team servers, committed to the repo |

**How layering works:**
- User config takes priority over project config
- When you enable/disable a project-defined server, a copy is saved to your user config with the new state
- The original project config remains unchanged (so teammates aren't affected)
- Project servers cannot be removed via CLI or UI—edit `.factory/mcp.json` directly

**OAuth tokens** are stored globally in your system keyring, not per-project. If you authenticate with a server in one project, you're authenticated everywhere.
