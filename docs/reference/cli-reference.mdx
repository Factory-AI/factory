...
- Browse and search configured MCP servers
- View tools provided by each connected server
- Enable/disable servers temporarily
- Authenticate with OAuth-enabled MCP servers
- Clear authentication credentials
- Add servers from a curated registry
- Remove user-configured servers

For CLI-based management (useful for scripting and automation), use:
...
**Configuration files:**
- **User config:** `~/.droid/mcp.json` - Your personal MCP servers
- **Project config:** `.factory/mcp.json` - Shared team servers (committed to repo)

Project-configured servers cannot be removed via CLI—edit `.factory/mcp.json` directly.
