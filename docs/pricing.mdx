...
Factory plans use **rolling rate limits** across three windows: **5-hour**, **weekly**, and **monthly**. Your plan's Standard Usage gives you access to all models and is consumed first. Growth and Enterprise plans are not affected by rate limit changes.
...
Growth plans are not affected by rate limit changes. [Contact Sales](https://factory.ai/contact) for Growth pricing.
...
No. You get more total monthly usage on every tier.
...
No, similar to the previous plan they do not roll over month to month.
...
Standard, always. Extra Usage is never touched while you have Standard Usage available.

### Rate Limits

**What happens mid-session when I hit a limit?**
The current request finishes, then subsequent requests get refused with a rate-limit error in the CLI or Web UI. Run `/limits` to toggle Droid Core or Extra Usage and retry immediately. No need to reopen the session.
...
**What if I'm on Enterprise?**
Enterprise plans are not affected by this change.
